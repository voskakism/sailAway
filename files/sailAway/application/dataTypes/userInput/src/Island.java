...
class Island extends UserInput
{
	...
	public boolean contains(Coordinates apex) // point-in-polygon algorithm, zero-slope, a.k.a flat (y = latitude of apex) ray-casting implementation. Stubborn, deterministic approach. Somewhat inefficient.
	{
	...
	public double distanceFrom(Coordinates point)
	{
		...
		LinkedList<Double> shoreDistances = new LinkedList<Double>();
		double distanceFromShore = 0;
		double minimumDistance = 0;
		double distance = 0;
		
		for(Shore shore : shores){
			distanceFromShore = shore.distanceFrom(point);
			shoreDistances.add(new Double(distanceFromShore));
		}
		minimumDistance = shoreDistances.peekFirst();
		for(Double dist : shoreDistances){
