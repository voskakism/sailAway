	...
	public void paintComponent(Graphics g)
	{
		...
		if(AppConfiguration.getDebugStatus()){
			...
			if(seaRatioLessThanScreenRatio){
				...
				System.out.println("Theatre's containing rectangle's side ratio is LESS than canvas' side ratio.");
			} else{
				g2.drawRect(margin, margin, clipWidth, topPadding);
				g2.drawRect(margin, margin + canvasHeight - topPadding, clipWidth, topPadding);
				System.out.println("Theatre's containing rectangle's side ratio is GREATER than canvas' side ratio.");
			}
		}
