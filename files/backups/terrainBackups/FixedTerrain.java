...
class FixedTerrain extends Terrain
{
	...
	public void paintComponent(Graphics g)
	{
		...
		if(AppConfiguration.getDebugStatus()){
			...
			if(seaRatioLessThanScreenRatio){
				...
				System.out.println("Archipelago's containing rectangle's side ratio is LESS than canvas' side ratio, a.k.a vertical");
			} else{
				g2.drawRect(margin, margin, clipWidth, topPadding);
				g2.drawRect(margin, margin + canvasHeight - topPadding, clipWidth, topPadding);
				System.out.println("Archipelago's containing rectangle's side ratio is MORE than canvas' side ratio, a.k.a horizontal");
			}
		}
	...
	{
		private String name;
		private LinkedList<IntPair> trail;
		...
		{
			this.name = name;
			this.trail = new LinkedList<IntPair>();
			r = new Random();
		...
		LinkedList<IntPair> getTrail() {return trail;}
	...
	private class Beep
	{
		...
		Beep(Long mmsi, String name, IntPair position, double heading, double speed)
		{
			this.mmsi = mmsi;
			this.name = name;
		...
		String getName() {return name;}
		IntPair getPosition() {return position;}
		double getHeading() {return heading;}
		double getSpeed() {return speed;}
	}
}