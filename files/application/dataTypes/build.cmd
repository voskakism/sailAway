...
:: are removed from the "classes" folders of the above "packages" (sort of..); in other words, when a clean rebuild is attempted. 
::
:: After building "configuration", "exceptions" and "lowLevelTypes" uneventfully, the script gets to the 4th "package" ("userInput") that contains the BroadcastSequence datatype.
...
:: the actual Broadcast.class file is missing, as the script has not gotten to that "package" yet to compile sources, and since this is a clean build there is no old .class file.
::
:: A workaround:
