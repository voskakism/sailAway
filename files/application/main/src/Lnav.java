...
class Lnav implements Runnable
{
	...
	private final double CHECK_SPEED_DISTANCE =  80; //17500 / COLLIDER_RADIUS; // until we come up with a better solution for slowing down before a turn...
	...
	Lnav(Vessel vessel, TripParameters parameters, Itinerary route, Terrain map, EPServiceProvider engine, AtomicInteger tripsReady, Object lock)
	{
	...
	public void run()
	{
		...
		do{
			...
			System.out.println("Leg initial Delay. timeTaken: " + legInitialDelay + " milliseconds"); /////////////////////////////////////////////////////////////////
			...
			do{
				...
				if(distanceToApex <= CHECK_SPEED_DISTANCE){
					if(speed > turnSpeedCap){
						speed = speed * (distanceToApex / CHECK_SPEED_DISTANCE) + turnSpeedCap;
					}
				} else{
				...
				offCourse = currentLeg.distanceFrom(position);
				if(offCourse > (LANE_WIDTH / 2)){ // take more aggressive corrective measures, aiming to get back on the leg
					//System.out.println("AGGRESSIVE CORRECTION MODE: " + offCourse + " meters off course");///////////////////////////////////////////////////////////
					
					// corrective turn
					// 1: angle
					if(step > offCourse){
						correctiveTurnAngle = Math.toDegrees(Math.asin(offCourse/step));
						//System.out.println("  CORRECTIVE ANGLE  --->  " + correctiveTurnAngle);///////////////////////////////////////////////
					} else{
